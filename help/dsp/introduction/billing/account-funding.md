---
title: アカウント資金調達
description: DSPのアカウントへの資金調達に関する詳細をご覧ください。
feature: DSP Introduction
exl-id: 95e1fd75-ed38-41e3-a464-afe5e23c1c22
TQID: https://experienceleague.adobe.com/TYwqNENRaL6sceEASH8FlVhIs1wd9fRQjm1rJCTE1qc
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
feature_v2:
  - id: ee30758d-9ffe-4cd7-8f26-0d4394f041f6
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
topic_v2:
  - id: f4e6943a-c91a-4134-a2c7-f4f20cfff2f0
source-git-commit: 527ca2bb74de388c13ba1ce5bde3f8be1cead8d0
workflow-type: tm+mt
source-wordcount: 263
ht-degree: 0%

---

# Advertising DSPの口座残高

DSPは、次の2つの方法のいずれかでメディアとサービスの購入を顧客に支払います。

* クライアントは前払いの金額を提出し、その金額からゼロのバランスに達するまで資金がサービスやメディアに使用されます。
* Adobeの信用格付けは、

すべてのクライアントは、Adobeクレジットチームによって契約履行で評価され、割り当てられます。 DSPでは、前払いが行われていない限り、割り当てられたクレジットレベルに顧客アカウントを資金調達することがデフォルトで設定されます。 アカウントは、アカウント作成後24時間以内に資金調達されます。

顧客がクレジットや前払いに対して支出する場合、残高がゼロに達するまで、支出に応じてメディアやサービスの資金が減少します。 クレジットは、請求書が支払われた時点で調整されます。 Adobeの未払い請求書は、DSPの資金調達に影響を与える可能性があります。 お客様は、すべてのAdobe請求書のタイムリーな支払いに責任を負います。 使用可能な残高を超える資金が必要な場合は、Adobeアカウントチームにお問い合わせください。

クライアントアカウントの資金調達は、[!DNL DSP]でほぼリアルタイムで更新されます。 使用可能な残高を表示するには、アカウントの購買通貨で「[!UICONTROL Usable Funds]」と表示されます。[!UICONTROL Settings] > [!UICONTROL Account]に移動します。 [!UICONTROL General]および[!UICONTROL Admin] ユーザーロールを持つDSP ユーザーのみが[!UICONTROL Usable Funds]を表示できます。

![&#x200B; アカウントの使用可能な資金](/help/dsp/assets/account-usable-funds.png)

Adobeは、DSP [広告要件ポリシー](/help/policies/ad-requirements-policy.md)またはプライバシー条件への違反を含む理由により、アカウントがDSPへの支出をブロックする権利を留保します。 Adobeがアカウントをブロックする場合は、ブロックされた通知がアカウント設定に含まれます。 Adobe アカウントチームと協力して、ブロックの理由とエラーの修正方法を確認します。

>[!MORELIKETHIS]
>
>* [&#x200B; サポートされている通貨](/help/dsp/currency.md)
