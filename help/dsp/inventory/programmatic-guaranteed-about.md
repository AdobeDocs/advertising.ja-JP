...
* パブリッシャーが管理する広告は、Google Authorized Buyers SSP を通じて受け入れられます。
* SSP とパブリッシャーは配信 SLA を持っています。

PG 取引には、DSPが各入札リクエストにリクエストを返し、SSP を使用した配信 SLA を満たすために、PG のデフォルト配置と広告（パブリッシャーが管理する広告の場合は 1 x 1 ピクセル）が必要です。 必須の PG デフォルトの配置を設定したら、他の配置でも PG 契約のターゲットを設定できます。
