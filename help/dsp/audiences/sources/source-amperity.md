...
セグメントは、24 時間以内にDSPで使用可能になります。また、内で広告主に対して設定されたとおりに更新されます [!DNL Amperity]. セグメントの更新頻度に関係なく、セグメントへの追加は、デフォルトで 30 日後、または顧客が指定した有効期限が切れた後に期限切れになります。 から再プッシュしてセグメントを更新 [!DNL Amperity] 有効期限の前。 カスタムセグメントの有効期限をリクエストするには、Adobeアカウントチームにお問い合わせください。
