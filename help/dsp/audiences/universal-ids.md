...
1. [!UICONTROL Audience Targeting] セクションで、次の操作を行います。
   ...
   1. [!UICONTROL Included Audiences]設定で、ユーザーデータがユニバーサル ID （[!DNL RampIDs]や[!DNL UID2] IDなど）に変換されたセグメント、またはインポートした[!DNL AdFixus] IDを含むセグメントを選択します。
   ...
   1. [!UICONTROL Targeting]設定で：
      ...
      1. ターゲットにするユニバーサル ID タイプを選択します。
         ...
         設定には、サブオプション「[!UICONTROL AdFixus]」、「[!UICONTROL ID5]」、「[!UICONTROL RampID]」、「[!UICONTROL Unified ID2.0]」を含むオプション「[!UICONTROL Legacy IDs]」と「[!UICONTROL Universal ID]」が含まれます。 選択した地理的目標によって、使用可能なサブオプションが決まります。
...
## [!DNL RampID] ベースのセグメントとID5 ベースのセグメントのテストとデータ検証のベストプラクティス
...
* （[!DNL RampIDs]および[!DNL UID 2.0] IDの場合）サイト上のイベントと広告露出に一致するように、正しいユニバーサル ID パートナー固有のコードがweb サイトに実装されていることを確認します。 必要に応じて、[!DNL LiveRamp]または[!DNL ID5]担当者と連携してください。
...
* （ID [!DNL AdFixus]の場合） [!UICONTROL AdFixus ID] ソースが正しく設定されており、[!DNL AdFixus]がストリーミングセグメントであることを確認します。

* リーチが予想を下回っている場合は、オーディエンスセグメントロジックが詳細でないことを確認します。

* キャンペーン、パッケージ、配置の設定が正しいことを確認します。
