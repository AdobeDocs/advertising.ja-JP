---
title: 消費者の販売拒否レポートの取得
description: オプトアウト要求のために送信されたIDのレポートを取得する方法について説明します。
feature: CCPA, DSP Segments
exl-id: 94133ce3-4e2c-4a24-90c9-61bf10731668
TQID: https://experienceleague.adobe.com/m5dLZLZsdmNVIazfFlsPcP7Y5t1tphs1CPwJ5lxvHP0
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
feature_v2:
  - id: ee30758d-9ffe-4cd7-8f26-0d4394f041f6
subfeature_v2:
  - id: c193c532-b70e-4556-bde7-857186cbe140
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
topic_v2:
  - id: f4e6943a-c91a-4134-a2c7-f4f20cfff2f0
source-git-commit: 527ca2bb74de388c13ba1ce5bde3f8be1cead8d0
workflow-type: tm+mt
source-wordcount: 158
ht-degree: 0%

---

# 消費者の販売拒否レポートの取得

顧客がアカウントのオプトアウト要求のために送信したIDの月次レポートを取得できます。 過去3か月間に作成されたレポートを取得できます。 各リンクは7日間有効ですが、顧客が取得するたびに更新されます。

>[!NOTE]
>
>DSP [!DNL Trafficking API]を使用して、オプションで消費者の販売拒否レポートを取得できます。 手順については、[!DNL Trafficking API]のドキュメントを参照してください。 お客様の組織が[!DNL Trafficking API]を使用していないが、詳細については、Adobe アカウント チームにお問い合わせください。

1. メインメニューで、**[!UICONTROL Audiences]** > **[!UICONTROL Segments]**&#x200B;をクリックします。
1. セグメントリストの上で、**[!UICONTROL Access CCPA Reports]**&#x200B;をクリックします。
1. 利用可能な月次レポートのリンクをクリックして、タブ区切りのテキストファイルに抽出できるGZIP ファイルをダウンロードします。

>[!MORELIKETHIS]
>
>* [Adobe Advertisingのカリフォルニア州消費者プライバシー法に対するサポート：消費者の販売拒否サポート &#x200B;](/help/privacy/ccpa/ccpa-opt-out-of-sale.md)
>* [約[!UICONTROL CCPA Opt-out-of-Sale]個のセグメントとレポート &#x200B;](ccpa-opt-out-about.md)
>* [[!UICONTROL CCPA Opt-Out-of-Sale] セグメントを作成して実装](ccpa-opt-out-segment-create.md)
>* [&#x200B; オーディエンス管理について](audience-about.md)
