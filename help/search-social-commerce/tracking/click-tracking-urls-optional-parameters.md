...
次のすべてのパラメーターは、キーと値のペアとして指定する必要があります。1つの値のペアに複数のパラメーターを含めることができます。 例えば、キャンペーン名を挿入するには、`<your_parameter_name>={ef_campaign}`など`campaign={ef_campaign}`を使用します。 指定された一致タイプ名を使用して一致タイプを挿入するには、`<your_parameter_name>={ef_mt_broad:<value>}{ef_mt_exact:<value>}{ef_mt_phrase:<value>}`など`matchtype={ef_mt_broad:<b>}{ef_mt_exact:<e>}{ef_mt_phrase:<p>}`を使用します。 該当しない一致タイプは、結果のURLに表示されません。
...
| <code>{ef_sid}</code> | Search, Social, &amp; Commerceが広告ネットワークに割り当てる数値IDを挿入するには：<i>[!UICONTROL 3]</i>の[!DNL Google Ads]、<i>[!UICONTROL 10]</i>の[!DNL Microsoft Advertising]、<i>[!UICONTROL 45]</i>の[!DNL Meta]、<i>[!UICONTROL 86]</i>の[!DNL Yahoo! Display Network]、<i>[!UICONTROL 87]</i>の[!DNL Naver]、<i>[!UICONTROL 88]</i>の[!DNL Baidu]、<i>[!UICONTROL 90]</i>の[!DNL Yandex]、<i>[!UICONTROL 94]</i>の[!DNL Yahoo! Japan Ads]、<i>[!UICONTROL 105]</i>の[!DNL Yahoo Native] （非推奨）、<i>[!UICONTROL 106]</i>の[!DNL Pinterest]です。 |
...
| <code>{ef_matchtype}</code> | キーワード一致タイプを「Broad」、「Exact」、「Phrase」として挿入するには、次の手順を実行します。 「[!DNL Google Ads]」トラッキングメソッドを使用して、[!DNL Microsoft Advertising]と[!UICONTROL EF Redirect]に対して自動的に含まれます。 |
...
## Yahoo! Japan Adsのダイナミックトラッキングパラメーター
