---
title: パブリッシャーの推奨事項とインサイトのサポートについて
description: パブリッシャーのレコメンデーションとインサイトの表示と管理のサポートについて説明します。
exl-id: 8bad85e0-8c01-4177-a881-96dd9c4a5949
feature: Search Recommendations
source-git-commit: c53d79bd185630fb535c44bc05a4e0b7582f14ca
workflow-type: tm+mt
source-wordcount: '652'
ht-degree: 0%

---

# パブリッシャーの推奨事項とインサイトのサポートについて

*[!DNL Google Ads]および [!DNL Microsoft® Advertising] アカウント*

*のベータ版機能 [!DNL Microsoft® Advertising] アカウント*

[!DNL Google Ads] および [!DNL Microsoft® Advertising] recommendations と insights は、キャンペーンのパフォーマンスと効率を向上させるのに役立つ、広告ネットワークからの提案です。

* Each [!DNL Google Ads] レコメンデーションでは、アカウントのパフォーマンス履歴、キャンペーン設定、キャンペーン全体のトレンドに基づいて、アセットの追加から予算の増加まで、キャンペーンの様々なパフォーマンス面に関するカスタマイズされた提案を提供します [!DNL Google Ads].

* Each [!DNL Microsoft® Advertising] パフォーマンスインサイトとレコメンデーションでは、機械学習アルゴリズムとベストプラクティスに基づいて、キャンペーンのパフォーマンスを最適化するための変更点を提案します。

## この [!UICONTROL Recommendations & Publisher Insights] 表示

内 [!UICONTROL Insights & Reports] > [!UICONTROL Recommendations & Publisher Insights]では、以下のことが可能です。

* アカウントの行動に反映されていない、サポートされるすべてのレコメンデーションを一目で確認できます。 各エントリの情報には、レコメンデーションタイプや [!DNL Adobe] 推奨事項、影響を受ける指標、影響を受けるエンティティ、詳細へのリンク。 指標の増加が予測されると、緑色でハイライト表示されます。

  ![RECOMMENDATIONS UI](/help/search-social-commerce/assets/recommendations-ui.png "RECOMMENDATIONS UI")

  ビューを開くと、データをリアルタイムで使用できます。 データを更新するには、 ![更新](/help/search-social-commerce/assets/refresh.png "更新") ページの左下

* の場合 [Microsoft® 広告] アカウント。過去 30 日間に生成された各パフォーマンスインサイトを一目で確認できます。 [Microsoft® 広告] アカウント。 インサイトは、レコメンデーションと同様の情報を提供しますが、形式が異なります。 各インサイトには、日付、イシューの説明、影響を受けるエンティティ、根本原因（詳細へのリンクを含む場合があります）、提案されたアクションと、Microsoft® Advertising エディターを開くためのリンクが含まれます。このリンクから、特定のインサイトに基づいてアクションを実行できます。

* レコメンデーションに関する詳細を表示し、レコメンデーションを直接適用または却下します。

* 影響を受けたエンティティ、レコメンデーションを適用したユーザー、タイムスタンプなど、アカウントに適用された各レコメンデーションのログを表示します。

>[!NOTE]
>
>広告ネットワークのレコメンデーションとパフォーマンスインサイトはキャンペーンのパフォーマンスを向上させるのに役立ちますが、より広範な目標と合致しない場合もあります。 そのため、推奨事項やインサイトを実装する前に、Adobeアカウントチームに相談することをお勧めします。

## でサポートされるレコメンデーションタイプ [!DNL Google Ads]

| 推奨カテゴリ | レコメンデーションタイプ | 説明 |
| --- | --- | --- |
| [!UICONTROL Ads and extensions] （現在は「」[!DNL Ads and assets]」に含まれます [!DNL Google Ads]） | [!UICONTROL Call extension] | キャンペーンへの呼び出し拡張機能の追加 |
| | [!UICONTROL Callout extension] | キャンペーンへのコールアウト拡張機能の追加 |
| | [!UICONTROL Optimize ad rotation] | 最適化された広告回転を使用 |
| | [!UICONTROL Responsive search ad] | 新しいレスポンシブ検索広告の追加 |
| | [!UICONTROL Responsive search ad asset] | 広告へのレスポンシブ検索広告アセットの追加 |
| | [!UICONTROL Responsive search improve ad strength] | レスポンシブ検索広告の広告効果を向上させるための提案 |
| | [!UICONTROL Sitelink extension] | キャンペーンへのサイトリンク拡張機能の追加 |
| | [!UICONTROL Text ad] | 広告の提案を追加 |
| [!UICONTROL Automated campaigns] | [!UICONTROL DSA to performance max migration] | 動的検索広告のパフォーマンス最大化キャンペーンへの移行。 |
| | [!UICONTROL Dynamic image extension opt in] | アカウントの動的画像拡張機能を有効にする（次が可能） [!DNL Google Ads] 機械学習：最も関連性の高い画像を広告のランディングページから広告に自動的に追加します。 |
| | [!UICONTROL Improve performance max ad strength] | 「優れた」評価を得られるように、Performance MAX キャンペーンのアセットグループの有効性を高めます。 |
| | [!UICONTROL Performance max opt in] | Performance MAX キャンペーンをオプトインします。 |
| | [!UICONTROL Upgrade local campaign to performance max] | 従来のローカルキャンペーンをパフォーマンス最大化キャンペーンにアップグレード |
| | [!UICONTROL Upgrade smart shopping campaign to performance max] | 従来のスマートショッピングキャンペーンをパフォーマンス最大化キャンペーンにアップグレード |
| [!UICONTROL Bidding and budget] | [!UICONTROL Campaign budget] | 予算によって制限されているキャンペーンの修正 |
| | [!UICONTROL Enhanced CPC opt in] | 入札の相手 [!UICONTROL Enhanced CPC] |
| | [!UICONTROL Forecasting campaign budget] | 将来的に予算によって制限されると予想されるキャンペーンを修正 |
| | [!UICONTROL Forecasting set Target ROAS] | トラフィックの増加が予測される季節的なイベントの前に予算を引き上げ、入札戦略をから変更します [!UICONTROL Maximize Conversion Value] 対象： [!UICONTROL Target ROAS] |
| | [!UICONTROL Marginal ROI campaign budget] | ROI を向上させるためのキャンペーン予算の調整 |
| | [!UICONTROL Maximize clicks opt in] | 入札の相手 [!UICONTROL Maximize Clicks] |
| | [!UICONTROL Maximize conversions opt in] | 入札の相手 [!UICONTROL Maximize Conversions] |
| | [!UICONTROL Move unused budget] | 未使用の制約付き予算への移動 |
| | [!UICONTROL Raise Target CPA bid too low] | を上げる [!UICONTROL Target CPA] 推奨される金額（低すぎてコンバージョン数が少ない、またはまったくない場合） |
| | [!UICONTROL Target CPA opt in] | 入札の相手 [!UICONTROL Target CPA] |
| | [!UICONTROL Target CPA raising] | を上げる [!UICONTROL Target CPA] 基準： [!DNL Google Ads] 過去のコンバージョンから計算された予測。 |
| | [!UICONTROL Target ROAS lowering] | を下げる [!UICONTROL Target ROAS] 基準： [!DNL Google Ads] 過去のコンバージョンから計算された予測。 |
| | [!UICONTROL Target ROAS opt in] | 入札の相手 [!UICONTROL Target ROAS] |
| [!UICONTROL Keywords and targeting] | [!UICONTROL Display expansion opt in] | ディスプレイ拡張を使用するようにキャンペーンを更新してリーチを拡大 |
| | [!UICONTROL Keyword] | 新しいキーワードを追加 |
| | [!UICONTROL Search partners opt in] | ～とのリーチを拡大する [!DNL Google] パートナーを検索 |
| | [!UICONTROL Use broad match keyword] | 完全に自動化されたコンバージョンベースの入札を使用して、コンバージョンベースのキャンペーンに部分一致を使用します |

## でサポートされるレコメンデーションタイプ [!DNL Microsoft Advertising]

| 推奨カテゴリ | レコメンデーションタイプ | 説明 |
| --- | --- | --- |
| [!UICONTROL Ads and extensions] | [!UICONTROL Responsive search ad] | 新しいレスポンシブ検索広告の追加 |
| [!UICONTROL Bidding and budgets] | [!UICONTROL Campaign budget] | 予算によって制限されているキャンペーンの修正 |
| [!UICONTROL Keywords and targeting] | [!UICONTROL Keyword] | すべてのソースから新しいキーワードを追加 |

>[!MORELIKETHIS]
>
>* [パブリッシャーの推奨事項とパフォーマンスインサイトを表示する](recommendation-view.md)
>* [パブリッシャーのレコメンデーションの適用または解除](recommendation-apply-dismiss.md)
>* [アカウントの公開者レコメンデーションログの表示](recommendation-view-log.md)
>* [パブリッシャーのレコメンデーションをポートフォリオで使用する際のベストプラクティス](recommendation-best-practices.md)
