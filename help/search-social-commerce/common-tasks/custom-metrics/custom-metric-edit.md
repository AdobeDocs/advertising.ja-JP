---
title: カスタム指標の編集
description: 標準指標から計算されるカスタム指標を編集する方法について説明します。
...
デフォルトのビューまたはカスタムビュー、またはレポートテンプレートの列として含まれているカスタム指標を編集する場合、次にビューを更新またはレポートを実行するときにそれぞれ新しい数式が使用されます。

>[!NOTE]
>
>複雑なカスタム指標の計算に時間がかかり、それを含むレポートやビュー（特にクリックスルーとビュースルーコンバージョン用の別々の列が含まれる場合）の生成に時間がかかります。

## （新しいUI）管理ビューからのカスタム指標の編集
...
## （従来のUI） レポートまたはレポートテンプレートフォームからカスタム指標を編集する

1. レポートまたはレポートテンプレートフォーム内で、「**[!UICONTROL Columns]**」タブに移動します。
...
## （従来のUI） [!UICONTROL Portfolios] ビューからカスタム指標を編集する

1. データテーブルの上で、![選択したビューを編集](/help/search-social-commerce/assets/view-settings.png "選択したビューを編集")をクリックします。
