...
| フィードデータには、広告で使用される列の新しい値が含まれます。 | 前のファイル：広告が含まれている価格=20<br><br>新規ファイル：同じ広告の場合、price=10 | 広告が [!DNL Microsoft® Advertising] 拡張テキスト広告、 [!DNL Yahoo! Japan ads]または [!DNL Yandex] 広告が変更され、既存の広告が削除されて、新しい広告が作成されます。<br><br>広告コピーが他の広告タイプに対して変更されたとき、または該当する列が [!DNL Google Ads] 広告パラメーター ({param1} または {param2}) を広告内に追加した場合、既存の広告が更新されます。 |
