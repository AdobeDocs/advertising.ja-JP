...
* Yahoo! Japan Ads アカウントでは、パラメーター {lpurl} を使用します。
...
**[!UICONTROL Base URL]:** （リンク先 URL のアカウントのみ）ユーザーが移動するページ。 サードパーティのリダイレクトやトラッキングコードを含めることができます。 Adobe広告コンバージョントラッキングサービスを使用し、キャンペーン設定に [!UICONTROL EF Redirect] 広告レベルでトラッキングを追加すると、検索、ソーシャル、コマースによって、独自のリダイレクトとトラッキングコードが広告に自動的に追加されます。
