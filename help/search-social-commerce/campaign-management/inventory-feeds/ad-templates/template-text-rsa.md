...
* *[!UICONTROL Content]:* コンテンツ（ディスプレイ）ネットワークリストにプレースメントの入札を配置します。 **メモ：** テンプレートを使用してプレースメントを作成することはできません。 このオプションを選択する場合は、各広告グループにプレースメントを作成し、バルクシートまたは広告グループとプレースメント設定を使用して、表示ネットワーク上で各広告グループのターゲットにするページを [!UICONTROL Search] > [!UICONTROL Campaigns] ビューで指定します。
