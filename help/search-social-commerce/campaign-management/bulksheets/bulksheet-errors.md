...
Search、Social、および Commerce は、バルクシート操作中に次の 2 種類のエラーファイルを生成します。
...
|  | [!UICONTROL The number of creatives under an AdGroup should not exceed 50] | 各広告グループには最大 50 個の広告を含めることができ、このバルクシートには 50 を超える広告が含まれます。 広告の数を減らします。 |
...
| Sitelink | [!UICONTROL Sitelinks Cannot update more than 10 site links per campaign] | ([!DNL Yandex] ) メッセージが不正確である。各キャンペーンには、最大 4 つ（10 個ではなく）のサイトリンクを含めることができ、この一括送信シートには 4 個を超える情報が含まれます。 一部のサイトリンクを削除します。 |
...
| 除外 | [!UICONTROL Other than status is modified] | 除外のステータス (「[!UICONTROL Active]&quot;または&quot;[!UICONTROL Deleted]」) をクリックします。 |
