...
>* （CPCまたはeCPC入札戦略を使用したキャンペーン）入札制約がポートフォリオレベルの入札制限と競合する場合、その制約はポートフォリオレベルの入札制限を上書きします。 例えば、ポートフォリオの最低入札額が5 USDであるが、ポートフォリオ内の入札単位を3 USDの最低入札額に制限する場合、入札単位は3 USD以上に入札されます。 ただし、制限付き入札単位の全体的な支出は、ポートフォリオの[ 「制約を中心に支出」パラメーター](#spend-around-constraints)によって決定されます。
>* 制約はベース入札で動作します。 基本入札に対する入札調整（モバイルデバイス上のエンドユーザーの入札の引き上げなど）は、入札を制約の許容範囲から外すことができます。 例えば、制約で最大CPCが6 USDである場合、基本入札は既に6 USDであり、ポートフォリオはモバイルデバイスの入札調整を50% ～ 60%で自動最適化するため、最大CPCは6 USDではなく9.00 ～ 9.60 USDになります。
