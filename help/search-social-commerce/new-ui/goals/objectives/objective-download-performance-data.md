---
title: （新しいUI）目的のパフォーマンスデータのダウンロード
description: スプレッドシートファイルで目的をダウンロードする方法を説明します。
...
関連付けられているポートフォリオの数、クリック数、クリック数、コスト、インプレッション単価（CPC）、およびインプレッション数を、一度に[!DNL Microsoft Excel] ワークブック （XLSX ファイル）形式のファイルにダウンロードできます。

## フィルタリングされたデータ行を含むレポートを生成する

1. メインメニューで、**[!UICONTROL Goals]>[!UICONTROL Objectives]**&#x200B;をクリックします。

1. データをダウンロードする目的を指定します。

   * 特定の目的のデータをダウンロードするには、目的の横にあるチェックボックスをオンにします。
...
1. [!UICONTROL Grid Reports]設定で、一意のレポート名を入力し、**[!UICONTROL Generate]**&#x200B;をクリックします。
   ...
   デフォルトでは、ファイルの名前は「objective_YYYYMMDD_NNNNN」です。ここで、「NNNN」は順次ジョブ番号（「objective_20250402_1326」など）です。
