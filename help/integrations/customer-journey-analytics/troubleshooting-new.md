...
| （Viewthrough インタラクション呼び出し後にExperience Platformでビュースルーイベントが発生しない場合） ビュースルーインタラクション呼び出しで広告主IDが送信されない | WebSDK拡張機能の設定の[!UICONTROL Advertiser] セクションで広告主が設定され、有効になっていることを確認してから、ライブラリを再構築して再公開します。 |
