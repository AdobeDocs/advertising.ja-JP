...
* テストが実行されたAdobe Advertisingアカウント、キャンペーンまたは広告グループに固有のパネルを <!-- only applicable entities? --> ワークスペース内に作成します。 概要ビジュアライゼーションを使用すると、[!DNL Target] のテストパフォーマンスと同じレポートにAdobe Advertising指標を表示できます。
