...
* サーバー側挿入機能が実装されていない場合：
   ...
   * 検索、ソーシャル、コマースのお客様：
      ...
      * その他すべての広告ネットワーク上の広告の場合は、AMO ID パラメーターを [アカウントレベルの追加パラメーター](/help/search-social-commerce/campaign-management/accounts/ad-network-account-manage.md){target="_blank"}：をベース URL に追加します。
...
内の任意のレポートまたは監査 [!DNL Analytics]を使用する場合、ベストプラクティスは、AMO ID を対応するインスタンスと共に使用することです。 詳しくは、[次のクリックスルーデータ検証： [!DNL Analytics for Advertising]](data-variances.md#data-validation)」(「 [!DNL Analytics] そしてAdobe Advertising」
