...
description: Advertising CreativeのHTML 5 クリエイティブの仕様を参照します。
...
# HTML5 Advertising Creativeのクリエイティブの仕様

このドキュメントでは、[!DNL Creative] 内のHTML5 クリエイティブの要件と API サポートの概要を説明します。 API を使用すると、クリエイティブの配信時に属性を設定できるHTML5 のクリエイティブを開発できます。

## 対象範囲

[!DNL Creative] は、ページの設定済みのボーダー内に表示される非リッチメディアクリエイティブ用のHTML5 バナーをサポートしています。 次の種類のHTML5 クリエイティブを使用できます。
...
* **HTML5:** クリエイティブの作成時およびトラフィック作成時に設定できるランディングページ URL を最大 5 つサポートします。
...
* ZIP ファイルには、[!DNL Creative] JavaScript ライブラリへの参照を含んだ少なくとも 1 つのHTMLファイル（メインHTMLの表示ファイル）が含まれている必要があります。 メインHTMLファイルは、ルートフォルダーまたはサブフォルダーのいずれかに配置できます。

* メインのHTMLファイルには、特殊文字が含まれていない限り、任意の名前を付けること `index.html` できます（推奨されます）。

* 最終クリエイティブのレンダリングに必要なサポート対象アセットはすべて、フォルダー表示ファイルと同じHTMLー内か、メインフォルダーのサブフォルダー内にある必要があります。

* クリエイティブに、そのクリエイティブで参照されていないファイルを含めないでください。

### Advertising CreativeJavaScript ファイルを含める

メインHTMLファイル（他のファイルは含まない）には、JavaScript ファイル `AMOLibrary.js` への参照を含める必要があります。 次のアドレスを使用して、`<head>` セクションの最初の行にあるファイルを呼び出します。
...
#### 静的HTMLでのクリックスルー URL のサポート 5

##### `amo.registerClick(clkVar, clkUrl)`

は、クリックスルー URL と、各 URL を参照するために使用される関連パラメーター（`clickTag` と呼ばれます）を登録します。 これにより、クリックの追跡を追加する場所を [!DNL Creative] 広告サーバーに通知します。 この API を使用して、最大 5 つのクリックタグ変数を登録し、それぞれに対応するランディングページ URL を登録できます。

>[!NOTE]
>
>HTML5 クリエイティブに含める静的 URL は、ローカルテスト目的でのみ使用され、上書きされます。 HTML5 クリエイティブをアップロードする場合、`clickTag` 変数ごとにデフォルトのランディングページを定義します。 アップロードしたHTML5 クリエイティブを広告エクスペリエンスに割り当てる際に、各 `clickTag` 変数のデフォルトのランディングページを上書きし、エクスペリエンスを保存す [!DNL Creative] 際に URL にクリックの追跡を追加することもできます。
...
メインHTMLファイルの `<head>` セクションにある `amo.registerClick()` を呼び出します。
...
メインHTMLファイルの `<body>` セクションにある `amo.onAdClick()` を呼び出します。
...
#### フレキシブルHTMLでのクリックスルー URL のサポート 5
...
* ランディングページのドメインとそのサブドメインは、35 文字未満にする必要があります。 **メモ：** 最終的なランディングページの URL は、HTML5 アセット自体ではなくDSPで定義されます。
...
### シンプルなHTML5 クリエイティブのHTMLファイルの例（index.html）
...
### 静的HTML5 クリエイティブのHTMLファイル（index.html）の例
