...
| 日付 | 機能 | 説明 | 詳細情報 |
| ---- | ------- | ----------- | -------------------- |
| 2025 年 8 月 4 日（Pt） | Ad [!DNL] エクスペリエンスのオーディエンスのターゲット ] | 広告エクスペリエンスのオーディエンスターゲットを設定する際に、ノードに指定したターゲットの *[!UICONTROL Include any]* 用（デフォルト）または *[!UICONTROL Include all]* 用を選択できるようになりました。 このオプションは、インプレッションの対象として、ユーザーが指定されたオーディエンスの少なくとも 1 つに属するか、指定されたすべてのオーディエンスに属するかを決定します。<br><br> このオプションを使用すると、「[!UICONTROL Split targets to create nodes]」に対する既存のオプションは使用できなくなります。 | 「[ 最終レベルへのターゲットノードの追加 ](/help/creative/experiences/experience-target-node-add-final.md)」、「[ ノード間にターゲットノードの挿入 ](/help/creative/experiences/experience-target-node-add-inner.md)」、「[ 兄弟ターゲットノードの追加 ](/help/creative/experiences/experience-target-node-add-sibling.md)」を参照してください。 |
