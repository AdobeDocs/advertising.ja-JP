...
デフォルトでは、エクスペリエンスのクリエイティブローテーションは、全体的なクリックスルー率を最適化するためにアルゴリズムによって決定され、クリエイティブの最適化設定は、割り当てられたすべてのバンドルに適用されます。 クリエイティブのローテーションをカスタマイズすると、各バンドル内のクリエイティブを手動で実行し、指定したAdvertising DSPのカスタム目標、指定したバンドルシーケンス、各バンドルシーケンスでの指定した数のインプレッション数、または相対的な重みに従って、アルゴリズムで最適化できます。 特定のクリエイティブバンドルを指定した期間に連続して実行するようにスケジュールし、各スケジュールにカスタムのクリエイティブローテーション設定を適用することもできます。
